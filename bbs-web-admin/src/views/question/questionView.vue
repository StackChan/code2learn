...
<template>
    <div class="main blankBackground" >
        <div class="navbar">
          <el-button type="primary" plain @click="$router.push({path: state.flag === 1 ? '/admin/control/question/list' : '/admin/control/question/history'})">返回</el-button>
                    ...
                    <div class="editQuestion data-form">
                        <el-form label-width="120"  @submit.native.prevent>
                          ...
                          <el-form-item label="封面">
                            <image-upload  :limit=1 v-model="state.cover"/>
                          </el-form-item>
...
<!--                  标题-->
                ...
                <div class="main" :class="router.currentRoute.value.query.reportModule !=undefined && parseInt(router.currentRoute.value.query.reportModule as string) == 40 ? 'reportMark' : ''">
                  ...
                  <el-form-item label="封面">
                    <image-upload :disabled="true" :limit=1 v-model="state.cover"/>
                  </el-form-item>
<!--                  具体的问答内容  -->
...
<script lang="ts" setup>
    ...
    import { processErrorInfo,getLanguageClassName, updateURLParameter, getPageBasePath,getElementTop,verifyPermissionMenu } from '@/utils/tool';
    ...
    const {fileStorageSystem:store_fileStorageSystem,currentLanguage:store_currentLanguage} = storeToRefs(store)
   ...
   const handleEditAnswerNodes = (el: any) => {
    ...
    const beforeRouteEnter = (to: RouteLocationNormalized, from: RouteLocationNormalized, next: NavigationGuardNext) => {
      ...
      if(to.query.questionView_beforeUrl == undefined || to.query.questionView_beforeUrl==''){//前一个URL
        ...
        let newFullPath = 0(to.fullPath,'questionView_beforeUrl',encrypt);
        ...
        .then((response: AxiosResponse) => {
            if(response){
                ...
                if(result){
                  ...
                  if (question.cover !== "" && question.cover !== null) {
                    state.cover.push(question.cover);
                  } else {
                    state.cover = [];
                  }
								nextTick(()=>{
    ...
    const editQuestion = () => {
        ...
        formData.append('cover', state.cover.length > 0 ? state.cover[0] : "");
        if(state.point != null){
   ...
   const showReplyLevel = (answerId:string,replyId:string) => {
    ...
    if(router.currentRoute.value.query.flag != undefined && router.currentRoute.value.query.flag != ''){
        state.flag = router.currentRoute.value.query.flag;
      }
