...
public class Question_TemplateManage {
	...
	private PageView<Question> question_SQL_Page(Forum_QuestionRelated_Question forum_QuestionRelated_Question,Map<String,Object> parameter,Map<String,Object> runtimeParameter,int maxResult){
		...
		// 模糊查询关键字
		if (forum_QuestionRelated_Question.getKeyword() != null && !forum_QuestionRelated_Question.getKeyword().trim().isEmpty()) {
			keyword = forum_QuestionRelated_Question.getKeyword().trim();
		}
		//排序
		...
		if(parameter != null && parameter.size() >0){
			for(Map.Entry<String,Object> paramIter : parameter.entrySet()) {
				...
				else if ("keyword".equals(paramIter.getKey())) {
					keyword = paramIter.getValue().toString().trim();
				}
			}
		}
		...
		if (keyword != null && !keyword.isEmpty()) {
			...
			params.add("%" + keyword + "%");
			jpql.append(" or o.summary like ?" + (params.size() + 1) + ")");
			params.add("%" + keyword + "%");
		}
...
//				}
				...
				//处理文件防盗链
				if(question.getContent() != null && !"".equals(question.getContent().trim()) && systemSetting.getFileSecureLinkSecret() != null && !"".equals(systemSetting.getFileSecureLinkSecret().trim())){
					List<String> serverAddressList = fileManage.fileServerAllAddress(request);


					//解析上传的文件完整路径名称
					Map<String,String> analysisFullFileNameMap = questionManage.query_cache_analysisFullFileName(question.getContent(),question.getId(),serverAddressList);
					if(analysisFullFileNameMap != null && analysisFullFileNameMap.size() >0){
						...
						for (Map.Entry<String,String> entry : analysisFullFileNameMap.entrySet()) {
							...
							newFullFileNameMap.put(entry.getKey(), Configuration.getUrl(request)+ SecureLink.createDownloadRedirectLink(entry.getKey(),entry.getValue(),systemSetting.getFileSecureLinkSecret()));
						}

						Integer topicContentUpdateMark = questionManage.query_cache_markUpdateTopicStatus(questionId, Integer.parseInt(
								RandomStringUtils.randomNumeric(8)));
						//生成处理'上传的文件完整路径名称'Id
						String processFullFileNameId = questionManage.createProcessFullFileNameId(questionId,topicContentUpdateMark,newFullFileNameMap);

						question.setContent(questionManage.query_cache_processFullFileName(question.getContent(),"question",newFullFileNameMap,processFullFileNameId,serverAddressList));
				...
				//处理视频播放器标签
				if(question.getContent() != null && !"".equals(question.getContent().trim())){
					Integer topicContentUpdateMark = questionManage.query_cache_markUpdateTopicStatus(questionId, Integer.parseInt(RandomStringUtils.randomNumeric(8)));

					//生成处理'视频播放器'Id
					String processVideoPlayerId = mediaProcessQueueManage.createProcessVideoPlayerId(questionId,topicContentUpdateMark);


					//处理视频播放器标签
					String content = mediaProcessQueueManage.query_cache_processVideoPlayer2(Configuration.getUrl(request),question.getContent(),processVideoPlayerId+"|"+topicContentDigest_link,systemSetting.getFileSecureLinkSecret());
