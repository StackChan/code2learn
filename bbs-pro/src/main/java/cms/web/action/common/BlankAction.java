...
public class BlankAction extends AbstractController{
	...
	protected ModelAndView handleRequestInternal(HttpServletRequest request, HttpServletResponse response) throws Exception {
	  ...
	  // 判断Url并分发到不同的Action，并统一控制转向
	    ...
	    String dirName = templateService.findTemplateDir_cache();//获取到layout
	    ...
	    if(currentURI != null && !"".equals(currentURI)){
	        ...
	        List<Layout> layoutList = templateService.findLayout_cache(templatesDir,4);//前后端分离为api 4.空白页
	        		...
	        		if(URI.toLowerCase().equals(layout.getReferenceCode().toLowerCase())){
	        			...
	        			if(layout.getReturnData().equals(0)){//返回数据为html方式
	        	    		...
	        	    		for(Forum forum: forumList){
	        	    			Object obj = templateMain.templateObject(forum,parameter,runtimeParameter);//所有的api集合
	        	    			if(obj != null){
