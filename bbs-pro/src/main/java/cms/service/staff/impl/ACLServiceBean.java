...
public class ACLServiceBean extends DaoSupport implements ACLService {
	...
	public List<PermissionMenu> findAllPermissionMenu() {
		Query query = em.createQuery("select b.name,b.url,c.methods from SysPermissionResources a,SysResources b,SysPermission c where a.resourceId=b.id and c.id=a.permissionId and (b.urlParentId is null or b.urlParentId ='')");
